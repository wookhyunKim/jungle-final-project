...
const Profile = ({ role, btnName, children, code, name, nickname, type }) => {
    ...
    const [inputCode, setInputValue] =
        role !== "HOST" ? useState("") : [null, () => {}]; // role이 "HOST"일 때 빈 상태 반환
    ...
    const handleMkRoom = (event) => {
        ...
        //players 전달
        if (role === "HOST") {
            navigate(targetUrl, { state: { players, code, nickname } });
        } else {
            navigate(targetUrl, { state: { nickname, inputCode } });
        }
                ...
                {role === "HOST" ? (
                    ...
                    <>
                        <div className="input-form">
                            ...
                            ></input>
                            <Button onClick={handleMkRoom}>{btnName}</Button>
                        </div>
                    </>
                )}
            </div>
